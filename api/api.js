...
export const renderHTML = ({ content }) => {
  ...
  renderer.heading = (text, level) => {
    ...
    const html = `<p class="text-${Math.ceil(lvl).toFixed(0)}xl md:text-3xl mb-5">\n${text}\n</p>`
    return html
  }
  renderer.paragraph = (text) => {
    const html = `<p class="py-6">\n${text}\n</p>`
    return html
  }
  renderer.blockquote = (text) => {
    const html = `<blockquote class="border-l-4 border-teal-500 italic my-8 pl-8 md:pl-12">\n${text}\n</blockquote>`
    return html
  }
  renderer.link = (href, title, text) => {
    const html = `<a name="${title}" title="${title}" class="text-gray-800 hover:text-teal-500 no-underline border-b-2 border-teal-500" href="${encodeURI(href)}">\n${text}\n</a>`
    return html
  }
  ...
  renderer.code = (code, codeType, escaped) => {
    const rID = `A${(Math.random() * 100000).toFixed(0)}`
  ...
  const htmlR = marked(content || '<div></div>', options)
  const cleanContent = sanitizeHtml(htmlR, {
    allowedAttributes: {
      a: ['href', 'name', 'target'],
      img: ['src'],
      '*': ['style', 'class', 'id']
...
export const getReadingTime = (v) => {
  const val = readingTime(v).minutes
  return val < 1 ? 1 : val.toFixed(0)
}

export const getLokProfSettings = async () => {
  const settingsURL = '/blogsettings?slug=lok-profile'
  const resp = await axios({
    method: 'GET',
    baseURL,
    url: settingsURL
  })
  const raw = resp.data[0].json
  return raw
}
